...
REPO_SHIP_UPDATE = (
    "Updating ship {ship_id}: setting {component} to {component_id} in {db_name}"
)
REPO_SHIP_UPDATE_SUCCESS = "Successfully updated ship {ship_id}"
REPO_COMPONENT_FIND_START = (
    "Finding component {component_type}: {component_id} in database: {db_name}"
)
REPO_COMPONENT_FIND_SUCCESS = "Component found: {component_id}"
REPO_COMPONENT_FIND_NOT_FOUND = (
    "Component not found: {component_id} in database: {db_name}"
)
REPO_COMPONENT_FIND_ALL = (
    "Retrieving all components from table: {component_table} in database: {db_name}"
)
REPO_COMPONENT_FIND_ALL_SUCCESS = "Retrieved {count} components from {component_table}"
REPO_COMPONENT_UPDATE = (
    "Updating component {component_id}: setting {param_name} to {param_value}"
)
...
SERVICE_GET_COMPONENT_START = (
    "Service: Getting {component_type} component {component_id} from {db_name}"
)
SERVICE_GET_COMPONENT_SUCCESS = (
    "Service: Successfully retrieved component {component_id}"
)
...
SERVICE_COMPARE_SHIPS_MATCH = (
    "Service: Ships have matching {component_type}: {component_id}"
)
SERVICE_COMPARE_SHIPS_DIFFER = (
    "Service: Ships have different {component_type}: {orig} vs {changed}"
)
SERVICE_COMPARE_PARAMS_START = (
    "Service: Comparing parameters for component {component_id}"
)
SERVICE_COMPARE_PARAMS_MATCH = (
    "Service: All parameters match for component {component_id}"
)
SERVICE_COMPARE_PARAMS_DIFFER = (
    "Service: Parameter '{param}' differs: {orig_value} vs {changed_value}"
)
SERVICE_UPDATE_SHIP = (
    "Service: Updating ship {ship_id} component {component_type} to {component_id}"
)
SERVICE_UPDATE_COMPONENT = (
    "Service: Updating component {component_id} parameter {param_name}"
)

# Test setup logging
TEST_RANDOMIZE_SHIP_START = "Randomizing ship: {ship_id}"
TEST_RANDOMIZE_SHIP_COMPLETE = (
    "Ship {ship_id} randomized: {component} set to {component_id}"
)
TEST_RANDOMIZE_COMPONENT_START = "Randomizing component: {component_id}"
TEST_RANDOMIZE_COMPONENT_COMPLETE = (
    "Component {component_id} randomized: {param} set to {value}"
)
